"""
bnb_collector_v1.py - Syndicate BNB Smart Chain Collector V1
...
- Throttling/backoff na BscScan API pozivima
- V6 intel polja u training record

Instalacija: pip install requests
Pokretanje:  python bnb_collector_v1.py [<contract_address>]
"""
...
# Config
# ---------------------------------------------------------------------------
BscScan_API_KEY = clean_env_value("BSCSCAN_API_KEY") or clean_env_value("BSC_SCAN_API_KEY")
BscScan_API     = clean_env_value("BSCSCAN_API", "https://api.bscscan.com/api")
BNB_RPC          = clean_env_value("BNB_RPC") or clean_env_value("BSC_RPC", "https://bsc-dataseed.binance.org/")
GOPLUS_TOKEN_SECURITY_API = clean_env_value(
    "GOPLUS_TOKEN_SECURITY_API",
    "https://api.gopluslabs.io/api/v1/token_security/{chain_id}",
)
GOPLUS_API_TIMEOUT = int(os.getenv("GOPLUS_API_TIMEOUT", "12"))

OUTPUT_FILE       = "syndicate_train_bnb_v1.jsonl"
...
RUN_UNTIL_DATE = os.getenv("RUN_UNTIL_DATE", "2026-07-01")
...
BNB_SCAN_LOG = Path(clean_env_value("BNB_SCAN_LOG", "bnb_scan_log.md"))
...
ACTIVITY_LOGGER_ADDRESS = clean_env_value("BNB_ACTIVITY_LOGGER_ADDRESS") or clean_env_value("BSC_ACTIVITY_LOGGER_ADDRESS")
REGISTRY_ADDRESS = clean_env_value("BNB_REGISTRY_ADDRESS") or clean_env_value("BSC_REGISTRY_ADDRESS")
...
GECKOTERMINAL_NEW_POOLS_URL = clean_env_value(
    ...
    "https://api.geckoterminal.com/api/v2/networks/bsc/new_pools?include=base_token,quote_token",
)
...
# Throttle: BscScan free tier ~5 req/s
_last_api_call = [0.0]
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [BNB-V1] [%(levelname)s] %(message)s",
    datefmt="%H:%M:%S",
)
...
def init_database() -> None:
    conn = _db_connect()
    if conn is None:
        log.info("DATABASE_URL nije postavljen; PostgreSQL upis je isključen.")
        return
    ...
    try:
        ...
        log.error("PostgreSQL init greška: %s", e)
    finally:
        conn.close()
...
def detect_cross_chain_match(deploy_ts: int, tx_amounts: list, holder_count: int, chain: str = "BNB") -> dict:
    result = {
        "pattern_hash": "",
...
def predict_lifecycle(intel: dict, creator_rug_rate: float) -> dict:
    ...
    if signals >= 4:
        ...
        result.update({"estimated_rug_minutes": 1440, "confidence": 0.20,
                        "prediction_text": "Monitor — weak signals detected"})
    return result
...
# ---------------------------------------------------------------------------
# V5 MODULE 5: Dev -> CEX Sweep (BNB known CEX wallets)
# ---------------------------------------------------------------------------
KNOWN_CEX_WALLETS_BNB = {
...
def detect_cex_sweep_BNB(deployer: str, deploy_timestamp: int) -> dict:
    result = {
        "sweep_to_cex": False,
        "cex_destination": "",
        "sweep_count": 0,
    ...
    for tx in txs:
        ...
        if to_addr in KNOWN_CEX_WALLETS_BNB:
            sweep_count += 1
            destinations.append(KNOWN_CEX_WALLETS_BNB[to_addr])
...
def detect_contract_backdoor_BNB(contract_address: str) -> dict:
    result = {
        "has_backdoor": False,
        "backdoor_functions": [],
    ...
    try:
        ...
        resp = requests.post(BNB_RPC, json=payload, timeout=RPC_TIMEOUT)
...
def calculate_rug_velocity_BNB(contract_address: str, deploy_timestamp: int) -> dict:
    result = {
        "velocity_score": 0.0,
        "is_fast_rug": False,
    ...
    try:
        ...
        # Grupiši po vremenskim prozorima od 5 minuta
        window = 300  # 5 minuta
...
def _goplus_bool(value) -> bool:
    if isinstance(value, bool):
        return value
    return str(value).strip().lower() in {"1", "true", "yes"}


def _goplus_float(value) -> float:
    try:
...
def detect_token_security_BNB(contract_address: str) -> dict:
    result = {
        "provider": "goplus",
        "available": False,
        "is_honeypot": False,
    ...
    try:
        url = GOPLUS_TOKEN_SECURITY_API.format(chain_id=56)
        resp = requests.get(
            url,
            params={"contract_addresses": contract_address.lower()},
            headers={"User-Agent": "RugBuster/1.0", "Accept": "application/json"},
            timeout=GOPLUS_API_TIMEOUT,
        )
        resp.raise_for_status()
        payload = resp.json()
        data = (payload.get("result") or {}).get(contract_address.lower())
        if not isinstance(data, dict):
            return result

        buy_tax = _goplus_float(data.get("buy_tax"))
        sell_tax = _goplus_float(data.get("sell_tax"))
        transfer_tax = _goplus_float(data.get("transfer_tax"))
        result.update({
            "available": True,
            "is_honeypot": _goplus_bool(data.get("is_honeypot")),
            "cannot_sell_all": _goplus_bool(data.get("cannot_sell_all")),
            "buy_tax": buy_tax,
            "sell_tax": sell_tax,
            "transfer_tax": transfer_tax,
            "tax_modifiable": any(_goplus_bool(data.get(key)) for key in (
                "slippage_modifiable",
                "personal_slippage_modifiable",
                "is_anti_whale",
                "anti_whale_modifiable",
            )),
            "blacklist": _goplus_bool(data.get("is_blacklisted")),
            "whitelist": _goplus_bool(data.get("is_whitelisted")),
            "trading_cooldown": _goplus_bool(data.get("trading_cooldown")),
            "hidden_owner": _goplus_bool(data.get("hidden_owner")),
            "open_source": _goplus_bool(data.get("is_open_source")),
            "proxy": _goplus_bool(data.get("is_proxy")),
            "owner_change_balance": _goplus_bool(data.get("owner_change_balance")),
            "owner_address": str(data.get("owner_address") or ""),
            "dex_liquidity": data.get("dex") if isinstance(data.get("dex"), list) else [],
        })

        if result["is_honeypot"]:
            result["risk_flags"].append("Honeypot detected")
        if result["cannot_sell_all"]:
            result["risk_flags"].append("Cannot sell all tokens")
        if sell_tax >= 0.5:
            result["risk_flags"].append(f"Extreme sell tax ({sell_tax:.0%})")
        elif sell_tax >= 0.2:
            result["risk_flags"].append(f"High sell tax ({sell_tax:.0%})")
        if buy_tax >= 0.2:
            result["risk_flags"].append(f"High buy tax ({buy_tax:.0%})")
        if transfer_tax >= 0.2:
            result["risk_flags"].append(f"High transfer tax ({transfer_tax:.0%})")
        if result["tax_modifiable"]:
            result["risk_flags"].append("Tax/slippage can be modified")
        if result["blacklist"]:
            result["risk_flags"].append("Blacklist controls detected")
        if result["whitelist"]:
            result["risk_flags"].append("Whitelist-only trading risk")
        if result["trading_cooldown"]:
            result["risk_flags"].append("Trading cooldown controls")
        if result["hidden_owner"]:
            result["risk_flags"].append("Hidden owner")
        if result["open_source"] is False:
            result["risk_flags"].append("Contract source not verified")
        if result["proxy"]:
            result["risk_flags"].append("Proxy contract")
        if result["owner_change_balance"]:
            result["risk_flags"].append("Owner can change balances")
    except Exception as exc:
        log.debug("  [V6] GoPlus security greška: %s", exc)
    return result

# ---------------------------------------------------------------------------
# BscScan API helpers (sa throttlingom)
# ---------------------------------------------------------------------------
...
def BscScan_get(params: dict) -> Optional[dict]:
    _throttle_api()
    ...
    for attempt, delay in enumerate(delays):
        try:
            resp = requests.get(BscScan_API, params=params, headers=headers, timeout=API_TIMEOUT)
            if resp.status_code == 429:
                log.warning("  [API] Rate limit (429) — čekam %ds (pokušaj %d/3)", delay, attempt + 1)
                time.sleep(delay)
                continue
            resp.raise_for_status()
            data = resp.json()
...
def get_token_info_BNB(contract_address: str) -> dict:
...
def get_erc20_metadata_rpc(contract_address: str) -> dict | None:
    if Web3 is None:
        return None
    web3 = Web3(Web3.HTTPProvider(BNB_RPC, request_kwargs={"timeout": RPC_TIMEOUT}))
...
def get_account_age_days_BNB(address: str) -> float:
...
def trace_funding_origin_BNB(deployer: str, depth: int = 3) -> dict:
    result = {
        "master_wallet": "",
        "hop_count": 0,
    ...
    for addr in chain_trace[1:]:
        age = get_account_age_days_BNB(addr)
        ages.append(age)
        time.sleep(0.3)
...
def get_deployment_latency_BNB(contract_address: str, deploy_timestamp: int) -> dict:
...
def analyze_transaction_entropy_BNB(contract_address: str) -> dict:
    result = {
...
def detect_wash_pattern_BNB(contract_address: str, deployer: str, deploy_timestamp: int) -> dict:
    result = {
...
def analyze_holder_cluster_BNB(contract_address: str) -> dict:
...
def run_cia_analysis_BNB(contract_address: str, deployer: str, deploy_timestamp: int) -> dict:
    log.info("  [CIA] Pokrenuta analiza za %s", contract_address[:12])
    intel = {}

    log.info("  [CIA] Tracing funding origin...")
    intel["funding"] = trace_funding_origin_BNB(deployer, depth=3) if deployer else {}
    time.sleep(0.5)

    log.info("  [CIA] Mjerim deployment latency...")
    intel["latency"] = get_deployment_latency_BNB(contract_address, deploy_timestamp)
    time.sleep(0.5)

    log.info("  [CIA] Analiziram transaction entropy...")
    intel["entropy"] = analyze_transaction_entropy_BNB(contract_address)
    time.sleep(0.5)

    log.info("  [CIA] Detektujem wash pattern...")
    intel["wash"] = detect_wash_pattern_BNB(contract_address, deployer, deploy_timestamp)
    time.sleep(0.5)

    log.info("  [CIA] Analiziram holder cluster...")
    intel["cluster"] = analyze_holder_cluster_BNB(contract_address)

    return intel


def run_v5_analysis_BNB(contract_address: str, deployer: str, deploy_timestamp: int,
    ...
    v5["cross_chain"] = detect_cross_chain_match(deploy_timestamp, tx_amounts, holder_count, "BNB")
    if v5["cross_chain"]["cross_chain_match"]:
        log.warning("  [V5] Cross-chain match: pattern viđen na %s", v5["cross_chain"]["match_chains"])
    ...
    v5["cex_sweep"] = detect_cex_sweep_BNB(deployer, deploy_timestamp)
...
def run_v6_analysis_BNB(contract_address: str, deployer: str, deploy_timestamp: int) -> dict:
    ...
    v6["backdoor"] = detect_contract_backdoor_BNB(contract_address)
    ...
    v6["velocity"] = calculate_rug_velocity_BNB(contract_address, deploy_timestamp)
    if v6["velocity"]["is_fast_rug"]:
        log.warning("  [V6] Fast rug detected (score=%.2f)", v6["velocity"]["velocity_score"])

    log.info("  [V6] Token security...")
    v6["security"] = detect_token_security_BNB(contract_address)
    if v6["security"].get("risk_flags"):
        log.warning("  [V6] Security flags: %s", v6["security"]["risk_flags"][:5])
...
def v6_success_rate(v5: dict, v6: dict) -> str:
    ...
    checks = [
        ...
        v6.get("security", {}).get("available", False),
    ]
    ok = sum(checks)
...
def classify_BNB_token_v6(token_info: dict, cia_intel: dict, v5: dict, v6: dict,
    ...
    for sec_flag in security.get("risk_flags", [])[:6]:
        flags.append(sec_flag)
    ...
    # Scoring. Low holders, fresh deployers and empty wallets are normal for a
    # brand-new BNB token, so they stay auxiliary unless a real EVM/CIA signal
    # confirms the risk.
    hard_danger_count = sum([
        wash.get("wash_detected", False),
        wash.get("dev_sold_fast", False),
        ...
        security.get("cannot_sell_all", False),
        security.get("sell_tax", 0.0) >= 0.5,
        security.get("owner_change_balance", False),
    ])
    primary_risk_count = sum([
        funding.get("all_fresh", False),
        latency.get("is_sniped", False),
        backdoor.get("has_mint_function", False),
        backdoor.get("is_proxy", False),
        conc.get("concentration_risk") == "HIGH",
        style.get("name_scam_score", 0) >= 25,
        backdoor.get("backdoor_risk_score", 0) >= 40,
        security.get("sell_tax", 0.0) >= 0.2,
        security.get("buy_tax", 0.0) >= 0.2,
        security.get("transfer_tax", 0.0) >= 0.2,
        security.get("tax_modifiable", False),
        security.get("blacklist", False),
        security.get("whitelist", False),
        security.get("trading_cooldown", False),
        security.get("hidden_owner", False),
        security.get("proxy", False),
    ])
    auxiliary_risk_count = sum([
        funding.get("is_fresh_wallet", False),
        deployer_balance < 0.01,
        holder_count < 10,
        non_ascii_ratio > 0.35,
        owner_controls and holder_count < 25,
        security.get("open_source") is False,
    ])
    evidence_score = hard_danger_count * 4 + primary_risk_count * 2 + auxiliary_risk_count

    if (
        hard_danger_count >= 2
        or backdoor.get("has_drain_function", False)
        or backdoor.get("has_blacklist", False)
        or backdoor.get("backdoor_risk_score", 0) >= 80
        or security.get("is_honeypot", False)
        or security.get("cannot_sell_all", False)
        or security.get("sell_tax", 0.0) >= 0.8
        or security.get("owner_change_balance", False)
        or (hard_danger_count >= 1 and evidence_score >= 7)
    ):
        return "DANGER", flags
    elif (
        hard_danger_count >= 1
        or primary_risk_count >= 2
        or (primary_risk_count >= 1 and auxiliary_risk_count >= 2)
        or evidence_score >= 6
    ):
        return "WARN", flags
    else:
...
Token Security: available={security.get('available', False)} | honeypot={security.get('is_honeypot', False)} | sell_tax={security.get('sell_tax', 0.0)} | buy_tax={security.get('buy_tax', 0.0)} | flags={security.get('risk_flags', [])}"""

    cia_flags = f" CIA/V6 flags: {', '.join(risk_flags[:4])}." if risk_flags else ""
    if label == "DANGER":
        output = f"DANGER - High risk BNB token. Top flags: {', '.join(risk_flags[:3])}.{cia_flags} Deployer rug rate: {creator_stats['rug_rate']}%."
    elif label == "WARN":
        output = f"WARN - Moderate risk BNB token. Flags: {', '.join(risk_flags[:3])}.{cia_flags}"
    else:
        output = f"GOOD - Low risk BNB token. No major red flags. Deployer history: {creator_risk}."

    return {
        "instruction": "Analyze this BNB Smart Chain token and classify its risk level as DANGER, WARN, or GOOD.",
        "contract_address": contract_address.lower(),
        ...
        "v6_security_available": security.get("available", False),
        "v6_is_honeypot": security.get("is_honeypot", False),
        "v6_cannot_sell_all": security.get("cannot_sell_all", False),
        "v6_buy_tax": security.get("buy_tax", 0.0),
        "v6_sell_tax": security.get("sell_tax", 0.0),
        "v6_transfer_tax": security.get("transfer_tax", 0.0),
        "v6_tax_modifiable": security.get("tax_modifiable", False),
        "v6_security_blacklist": security.get("blacklist", False),
        "v6_security_whitelist": security.get("whitelist", False),
        "v6_security_open_source": security.get("open_source"),
        "v6_security_proxy": security.get("proxy", False),
        "v6_security_flags": security.get("risk_flags", []),
    }
...
def append_to_dataset(record: dict, output_path: Path) -> None:
    try:
        ...
        log.info("  -> Snimljeno [%s][BNB-V1] Rug rate: %s%%  Ukupno: %d",
...
def BNB_to_wei(amount: float) -> int:
    return int(amount * 10**18)


def wei_to_BNB(amount: int) -> float:
    return amount / 10**18


def load_daily_state() -> dict:
    default = {"date": utc_day(), "tokens": 0, "BNB_spent_wei": 0, "eur_spent": 0.0}
    if not BNB_STATE_FILE.exists():
        return default
    try:
        state = json.loads(BNB_STATE_FILE.read_text(encoding="utf-8"))
    ...
    state.setdefault("BNB_spent_wei", 0)
    state.setdefault("eur_spent", 0.0)
    return state


def save_daily_state(state: dict) -> None:
    BNB_STATE_FILE.write_text(json.dumps(state, indent=2, sort_keys=True), encoding="utf-8")


def daily_limits_open(state: dict) -> bool:
    if int(state.get("tokens", 0)) >= MAX_TOKENS_PER_DAY:
        return False
    if int(state.get("BNB_spent_wei", 0)) >= BNB_to_wei(MAX_BNB_TOTAL):
        return False
...
def fetch_BNB_eur_price() -> float:
    try:
        response = requests.get(
            "https://api.coingecko.com/api/v3/simple/price",
            params={"ids": "binancecoin", "vs_currencies": "eur"},
            timeout=10,
        )
        response.raise_for_status()
        price = float(response.json()["binancecoin"]["eur"])
        return price if price > 0 else BNB_EUR_PRICE_FALLBACK
    except Exception as exc:
        log.warning("Ne mogu dohvatiti BNB/EUR cenu, koristim fallback %.2f EUR: %s", BNB_EUR_PRICE_FALLBACK, exc)
        return BNB_EUR_PRICE_FALLBACK
...
def onchain_logging_ready() -> bool:
    ...
    if Web3 is None:
        ...
        return False
    if not (clean_env_value("BNB_LOG_PRIVATE_KEY") or clean_env_value("BSC_LOG_PRIVATE_KEY")):
        log.error("Nema BNB_LOG_PRIVATE_KEY/BSC_LOG_PRIVATE_KEY u .env; on-chain upis preskočen.")
        return False
    return True


def require_private_key() -> str:
    private_key = clean_env_value("BNB_LOG_PRIVATE_KEY") or clean_env_value("BSC_LOG_PRIVATE_KEY")
    ...
    if len(candidate) != 64 or any(ch not in "0123456789abcdefABCDEF" for ch in candidate):
        raise RuntimeError("BNB_LOG_PRIVATE_KEY nije validan hex private key: koristi 64 hex karaktera, bez navodnika/razmaka")
    return private_key
...
def apply_target_scan_fee(web3, tx: dict, modules_count: int) -> dict:
    if TARGET_BNB_PER_SCAN <= 0 or modules_count <= 0:
        return tx
    latest_block = web3.eth.get_block("latest")
    base_fee = int(latest_block.get("baseFeePerGas", 0) or 0)
    target_wei = BNB_to_wei(TARGET_BNB_PER_SCAN) // modules_count
...
def publish_module_payloads_onchain(payloads: list[dict], state: dict) -> list[dict]:
    ...
    web3 = Web3(Web3.HTTPProvider(BNB_RPC, request_kwargs={"timeout": 30}))
    if not web3.is_connected():
        raise RuntimeError("Ne mogu da se povežem na BNB RPC za on-chain logging")
    ...
    BNB_eur_price = fetch_BNB_eur_price()
    total_budget_wei = BNB_to_wei(MAX_BNB_TOTAL)
    sent: list[dict] = []
    ...
    for payload in payloads:
        ...
        projected_BNB = wei_to_BNB(projected_cost)
        projected_eur = projected_BNB * BNB_eur_price
        if int(state.get("BNB_spent_wei", 0)) + projected_cost > total_budget_wei:
            log.warning(
                "Ukupni BNB limit bi bio pređen (%0.6f/%0.6f). Stajem sa tx upisom.",
                wei_to_BNB(int(state.get("BNB_spent_wei", 0))),
                MAX_BNB_TOTAL,
            )
            break
        if float(state.get("eur_spent", 0.0)) + projected_eur > MAX_EUR_TOTAL:
            log.warning(
                "Ukupni EUR limit bi bio pređen (%0.2f/%0.2f EUR). Stajem sa tx upisom.",
        ...
        actual_BNB = wei_to_BNB(actual_cost)
        actual_eur = actual_BNB * BNB_eur_price
        state["BNB_spent_wei"] = int(state.get("BNB_spent_wei", 0)) + actual_cost
    ...
    if txs:
        for item in txs:
            lines.append(
                ...
                f"BNB={item['BNB_spent']:.8f}, eur={item.get('eur_spent', 0):.4f})"
            )
    else:
        lines.append("  - no on-chain tx sent")
    lines.append("")
    with BNB_SCAN_LOG.open("a", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
def send_telegram_alert_BNB(record: dict, txs: list[dict]) -> None:
    if not TELEGRAM_BOT_TOKEN:
        ...
        return
    label = str(record.get("label") or "UNKNOWN").upper()
    warn_alerts_enabled = clean_env_value("BNB_ALERT_WARN", "false").lower() in {"1", "true", "yes", "on"}
    if label != "DANGER" and not (label == "WARN" and warn_alerts_enabled):
        log.info("  [TELEGRAM] Preskočen %s alert; BNB šalje samo DANGER po defaultu.", label)
        return
    token = record.get("contract_address", "")
    ...
    response = requests.post(
        ...
        json={"chat_id": BNB_TELEGRAM_CHAT_ID, "text": message, "disable_web_page_preview": True},
        timeout=20,
    )
    try:
        response.raise_for_status()
        log.info("  [TELEGRAM] Alert poslat na %s", BNB_TELEGRAM_CHAT_ID)
    except requests.RequestException as e:
...
def process_token_BNB(token_data: dict, output_path: Path) -> dict | None:
    ...
    log.info("Novi BNB token: %s (%s) | %s", name, symbol, contract[:12])
    time.sleep(RATE_LIMIT_DELAY)

    token_info = get_token_info_BNB(contract)
    ...
    deployer_balance = get_BNB_balance(deployer) if deployer else 0.0
    log.info("  Deployer balance: %.4f BNB", deployer_balance)

    creator_stats = get_creator_stats(deployer)
    cia_intel = run_cia_analysis_BNB(contract, deployer, deploy_timestamp)
...
def parse_geckoterminal_pool_tokens(data: dict, source: str) -> list[dict]:
    ...
    for pool in pools:
        ...
        token_item = included.get(f"bsc_{token_addr}") or included.get(base_id) or included.get(quote_id) or {}
...
def get_geckoterminal_top_pool_tokens() -> list[dict]:
    ...
    for page in range(1, page_count + 1):
        try:
            url = f"https://api.geckoterminal.com/api/v2/networks/bsc/pools?include=base_token,quote_token&page={page}"
...
def get_new_token_deployments(from_block: int, to_block: int = 0) -> list:
    ...
    try:
        ...
        for chunk_start in range(0, len(batch), 20):
            ...
            resp = requests.post(BNB_RPC, json=chunk, timeout=30)
            results = resp.json()
        ...
        for item in all_blocks:
            ...
            for tx in txs:
                if tx.get("to") is None or tx.get("to") == "":
                    ...
                    rec_resp = requests.post(BNB_RPC, json=receipt_payload, timeout=RPC_TIMEOUT)
...
def poll_loop(output_path: Path) -> None:
    log.info("=" * 60)
    log.info("  Syndicate BNB Collector V6 — CIA + V5 + V6 Intel")
    ...
    log.info(
        "  Campaign     : until %s, %d tokens/day, %.2f EUR total, %.4f BNB total",
        RUN_UNTIL_DATE,
        MAX_TOKENS_PER_DAY,
        MAX_EUR_TOTAL,
        MAX_BNB_TOTAL,
    ...
    log.info("  Tx log       : %s", BNB_SCAN_LOG.absolute())
    ...
    while True:
        try:
            ...
            if not daily_limits_open(state):
                ...
                log.warning(
                    "Limit dostignut: tokens_today=%s/%s, BNB_total=%.6f/%.6f, eur_total=%.2f/%.2f. Nastavljam za %ds.",
                    state.get("tokens", 0),
                    MAX_TOKENS_PER_DAY,
                    wei_to_BNB(int(state.get("BNB_spent_wei", 0))),
                    MAX_BNB_TOTAL,
            ...
            if (
                ...
                log.info("Nađeno %d GeckoTerminal BNB top-pool refill tokena", len(top_pool_tokens))
            ...
            if pending_tokens and now >= next_scan_at:
                ...
                try:
                    ...
                    record = process_token_BNB(token_data, output_path)
                    if not record:
                        ...
                        send_telegram_alert_BNB(record, txs)
                        state = load_daily_state()
                        ...
                        log.info(
                            "Sledeći scan za %d min. Queue=%d, daily=%s/%s tokens, total %.6f/%.6f BNB, %.2f/%.2f EUR.",
                            delay_minutes,
                            len(pending_tokens),
                            state.get("tokens", 0),
                            MAX_TOKENS_PER_DAY,
                ...
                log.info("Queue=%d. Sledeći scan za još %ds.", len(pending_tokens), wait_left)

            current_block = new_block
            log.info("Čekam %ds...", POLL_INTERVAL)
            time.sleep(POLL_INTERVAL)
...
def scan_single_BNB(address: str) -> None:
    log.info("Skeniram BNB token: %s", address)

    token_info = get_token_info_BNB(address)
    deployer = ""
    ...
    deployer_balance = get_BNB_balance(deployer) if deployer else 0.0
    creator_stats = get_creator_stats(deployer)
    cia_intel = run_cia_analysis_BNB(address, deployer, deploy_timestamp)
