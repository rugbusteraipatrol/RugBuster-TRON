...
def analyze_holder_concentration_BASE(contract_address: str, total_supply: object = 0) -> dict:
    result = {
        "top5_pct": 0.0,
        "top1_pct": 0.0,
...
def resolve_contract_creation_BASE(address: str) -> dict:
...
def trace_funding_origin_BASE(deployer: str, depth: int = 3) -> dict:
    result = {
        ...
        "all_fresh": False,
        "wallet_ages_days": [],
        "first_hop": {"status": "unavailable", "address": None, "amount": None, "timestamp": None, "asset": "ETH", "error": "no_incoming_funding_found"},
    }
    current = deployer
...
def run_v6_analysis_BASE(contract_address: str, deployer: str, deploy_timestamp: int, token_info: dict | None = None) -> dict:
    ...
    v6["concentration"] = analyze_holder_concentration_BASE(contract_address, (token_info or {}).get("total_supply", 0))
...
def process_token_BASE(token_data: dict, output_path: Path) -> dict | None:
    ...
    creation = (
        ...
        else resolve_contract_creation_BASE(contract)
    )
    ...
    v6_intel = run_v6_analysis_BASE(contract, deployer, deploy_timestamp, token_info)
...
def scan_single_BASE(address: str) -> None:
    ...
    v6_intel = run_v6_analysis_BASE(address, deployer, deploy_timestamp, token_info)
